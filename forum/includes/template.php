...
class Template {
	...
	function Template($root = '.')
	{
		...
		$this->lang =& $lang;
		$this->use_cache = $ft_cfg['xs_use_cache'];
	}
	...
	{
		$filename = clean_filename(str_replace('/templates/', '', $filename));
	...
	function set_filename($handle, $filename, $xs_include = false, $quiet = false)
	{
		...
		if (empty($this->files_cache[$handle]) && !@file_exists($this->files[$handle])) {
			...
			die('Template->make_filename(): template file not found: <br /><br />' . hide_ft_path($this->files[$handle]));
		}
	...
	*/
	...
	function xs_startup()
	{
		...
		if (empty($this->xs_started)) {
			...
			$this->vars['LANG'] = isset($this->vars['LANG']) ? $this->vars['LANG'] : $ft_cfg['default_lang'];
