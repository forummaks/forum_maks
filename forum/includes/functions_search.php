...
function clean_words($mode, &$entry, &$stopword_list, &$synonym_list)
{
	...
	if ( $mode == 'post' )
	{
		...
		// Quoted text
		$entry = preg_replace('#\[quote:[a-z0-9]{10,}(=.*?)?\].*?\[\/quote:[a-z0-9]{10,}\]#', ' ', $entry);
	...
	if ( $mode == 'post' )
	{
		...
		// 'words' that consist of <4 or >20 characters are removed.
//	$entry = preg_replace('/[ ]([\S]{1,2}|[\S]{21,})[ ]/',' ', $entry);
		$entry = preg_replace('#\b([\w]{1,3}|[\w]{21,})\b#',' ', $entry);
	}
...
function split_words(&$entry, $mode = 'post')
{
	...
	$words_ary = array_unique(explode(' ', trim(preg_replace('#\s+#', ' ', $entry))));

	if (sizeof($words_ary) > $max_words_count)
	{
		$words_ary = array_splice($words_ary, 0, $max_words_count);
	}
...
function remove_search_post($post_id_sql)
{
	...
	$words_removed = false;
		$sql = "DELETE FROM " . SEARCH_WORD_TABLE . "
			...
			$words_removed = $db->affected_rows();
...
function username_search($search_match)
{
	...
	$template->assign_vars(array(
		'USERNAME' => (!empty($search_match)) ? clean_username($search_match) : '',
