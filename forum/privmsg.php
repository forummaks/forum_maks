...
$inbox_url = ( $folder != 'inbox' || $mode != '' ) ? '<a href="' . append_sid("privmsg.$phpEx?folder=inbox") . '">' . $lang['Inbox'] . '</a>' : '<span class="gen">' . $lang['Inbox'] . '</span>';
...
$outbox_url = ( $folder != 'outbox' || $mode != '' ) ? '<a href="' . append_sid("privmsg.$phpEx?folder=outbox") . '">' . $lang['Outbox'] . '</a>' : '<span class="gen">' . $lang['Outbox'] . '</span>';
...
$sentbox_url = ( $folder != 'sentbox' || $mode != '' ) ? '<a href="' . append_sid("privmsg.$phpEx?folder=sentbox") . '">' . $lang['Sentbox'] . '</a>' : '<span class="gen">' . $lang['Sentbox'] . '</span>';
...
$savebox_url = ( $folder != 'savebox' || $mode != '' ) ? '<a href="' . append_sid("privmsg.$phpEx?folder=savebox") . '">' . $lang['Savebox'] . '</a>' : '<span class="gen">' . $lang['Savebox'] . '</span>';
...
else if ( $mode == 'read' )
{
	...
	//qr
	$temp_url = append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . '=' . $user_id_from);
	$username_from_url = '<a href="' . $temp_url . '">' . $privmsg['username_1'] . '</a>';
	$temp_url = append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . '=' . $user_id_to);
	$username_to_url = '<a href="' . $temp_url . '">' . $privmsg['username_2'] . '</a>';

	$template->assign_vars(array(
		'MESSAGE_TO_URL' => $username_to_url,
		'MESSAGE_FROM_URL' => $username_from_url,
		'QR_SUBJECT' => ((!preg_match('/^Re:/', $post_subject)) ? 'Re: ' : '') . htmlspecialchars($post_subject),
	));
	//qr end
...
else if ( $submit || $refresh || $mode != '' )
{
	...
	if ( $submit && !$error )
	{
		...
		$template->assign_vars(array(
			'META' => '<meta http-equiv="refresh" content="10;url=' . append_sid("privmsg.$phpEx?folder=inbox") . '">')
		);
...
// orig		$msg = $lang['Message_sent'] . '<br /><br />' . sprintf($lang['Click_return_inbox'], '<a href="' . append_sid("privmsg.$phpEx?folder=inbox") . '">', '</a> ') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . append_sid("index.$phpEx") . '">', '</a>');
		$msg = $lang['Message_sent'] . '<br /><br />' . sprintf($lang['Click_return_inbox'], '<a href="' . append_sid("privmsg.$phpEx?folder=inbox") . '">', '</a> ') . sprintf($lang['Click_return_sentbox'], '<a href="' . append_sid("privmsg.$phpEx?folder=sentbox") . '">', '</a> ') . sprintf($lang['Click_return_outbox'], '<a href="' . append_sid("privmsg.$phpEx?folder=outbox") . '">', '</a> ') . sprintf($lang['Click_return_savebox'], '<a href="' . append_sid("privmsg.$phpEx?folder=savebox") . '">', '</a> ') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . append_sid("index.$phpEx") . '">', '</a>');
	...
	if ( $preview && !$error )
	{
		...
		{
			$preview_message = $preview_message . '<br clear="all" /><br /><br />_________________<br />' . $user_sig;
		}
...
$template->assign_vars(array(
	'BOX_NAME' => $l_box_name,
	'BOX_DESC' => ($folder == 'outbox') ? 'В папке <b>Исходящие</b> находятся отправленные, но еще не прочтённые получателем сообщения. В <b>Отправленные</b> они попадают только после того, как получатель их прочтет. Сообщения, находящиеся папке в <b>Исходящие</b>, можно отредактировать или удалить.' : '',
	'INBOX_IMG' => $inbox_img,
	'SENTBOX_IMG' => $sentbox_img,
	'OUTBOX_IMG' => $outbox_img,
	'SAVEBOX_IMG' => $savebox_img,
	...
	'INBOX_LIMIT_IMG_WIDTH' => (@$inbox_limit_img_length) ? $inbox_limit_img_length : '',
	'INBOX_LIMIT_PERCENT' => (@$inbox_limit_pct) ? $inbox_limit_pct : '',
