...
$inbox_url = ( $folder != 'inbox' || $mode != '' ) ? '<a href="' . append_sid("privmsg.php?folder=inbox") . '">' . $lang['Inbox'] . '</a>' : '<span class="gen">' . $lang['Inbox'] . '</span>';
...
$outbox_url = ( $folder != 'outbox' || $mode != '' ) ? '<a href="' . append_sid("privmsg.php?folder=outbox") . '">' . $lang['Outbox'] . '</a>' : '<span class="gen">' . $lang['Outbox'] . '</span>';
...
$sentbox_url = ( $folder != 'sentbox' || $mode != '' ) ? '<a href="' . append_sid("privmsg.php?folder=sentbox") . '">' . $lang['Sentbox'] . '</a>' : '<span class="gen">' . $lang['Sentbox'] . '</span>';
...
$savebox_url = ( $folder != 'savebox' || $mode != '' ) ? '<a href="' . append_sid("privmsg.php?folder=savebox") . '">' . $lang['Savebox'] . '</a>' : '<span class="gen">' . $lang['Savebox'] . '</span>';
...
else if ( $mode == 'read' )
{
	...
	if ( !$userdata['session_logged_in'] )
	{
		redirect(append_sid("login.$phpEx?redirect=privmsg.php&folder=$folder&mode=$mode&" . POST_POST_URL . "=$privmsgs_id", true));
	}
	...
	//
	$sql = "SELECT u.username AS username_1, u.user_id AS user_id_1, u2.username AS username_2, u2.user_id AS user_id_2, u.user_sig_bbcode_uid, u.user_posts, u.user_from, u.user_website, u.user_email, u.user_icq, u.user_regdate, u.user_viewemail, u.user_rank, u.user_sig, u.user_avatar, pm.*, pmt.privmsgs_bbcode_uid, pmt.privmsgs_text
	...
	$temp_url = append_sid("profile.php?mode=viewprofile&amp;" . POST_USERS_URL . '=' . $user_id_to);
...
else if ( $submit || $refresh || $mode != '' )
{
	if ( !$userdata['session_logged_in'] )
	{
		...
		redirect(append_sid("login.$phpEx?redirect=privmsg.php&folder=$folder&mode=$mode" . $user_id, true));
	}
	...
	if ( $submit && !$error )
	{
		...
		$template->assign_vars(array(
			'META' => '<meta http-equiv="refresh" content="10;url=' . append_sid("privmsg.php?folder=inbox") . '">')
		);
