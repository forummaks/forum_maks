...
//bt
$dl_search = FALSE;
$l_dl_search_desc = $dl_username = $dl_uid = $user_status = '';

// for compatibility with old params
if ($search_id == 'willdowns')
{
	$search_id = 'dl';
	$_GET['dl_status'] = DL_STATUS_WILL;
}
else if ($search_id == 'currdowns')
{
	$search_id = 'dl';
	$_GET['dl_status'] = DL_STATUS_DOWN;
}
else if ($search_id == 'pastdowns')
{
	$search_id = 'dl';
	$_GET['dl_status'] = DL_STATUS_COMPLETE;
}
else if ($search_id == 'cancdowns')
{
	$search_id = 'dl';
	$_GET['dl_status'] = DL_STATUS_CANCEL;
}

if (isset($_GET['dl_user_id']) && !isset($_GET['dl_uid']))
{
	$_GET['dl_uid'] = $_GET['dl_user_id'];
}

// get search params
if ($search_id == 'dl' || isset($_GET['dl_search']))
{
	if (!isset($_GET['dl_status']))
	{
		message_die(GENERAL_ERROR, 'Invalid search request');
	}

	$dl_status = intval($_GET['dl_status']);

	if ($dl_status == DL_STATUS_WILL)
	{
		$user_status = DL_STATUS_WILL;
		$l_dl_search_desc = $lang['Search_DL_Will'];
	}
	else if ($dl_status == DL_STATUS_DOWN)
	{
		$user_status = DL_STATUS_DOWN;
		$l_dl_search_desc = $lang['Search_DL_Down'];
	}
	else if ($dl_status == DL_STATUS_COMPLETE)
	{
		$user_status = DL_STATUS_COMPLETE;
		$l_dl_search_desc = $lang['Search_DL_Complete'];
	}
	else if ($dl_status == DL_STATUS_CANCEL)
	{
		$user_status = DL_STATUS_CANCEL;
		$l_dl_search_desc = $lang['Search_DL_Cancel'];
	}
	else
	{
		message_die(GENERAL_ERROR, 'Invalid dl_status');
	}

	$l_dl_search_desc .= ' '. $lang['Search_DL'];

	if (isset($_GET['dl_uid']))
	{
		...
		$dl_username = $row['username'];
		$l_dl_search_desc .= " ($dl_username)";
	}
	else
	{
		$dl_uid = $userdata['user_id'];
		$dl_username = $userdata['username'];
	}
...
//if (                       $search_id == 'newposts' || $search_id == 'egosearch' || $search_id == 'unanswered' || $search_keywords != '' || $search_author != '' )
	if ( $search_id == 'dl' || $search_id == 'newposts' || $search_id == 'egosearch' || $search_id == 'unanswered' || $search_keywords != '' || $search_author != '' || $search_id == 'bookmarks')
	{
		...
		else if ($search_id == 'dl')
		{
			...
			if ($auth_sql != '')
			{
				$sql = 'SELECT t.topic_id, f.forum_id
					FROM '. TOPICS_TABLE .' t, '. BT_USR_DL_STAT_TABLE .' dl, '. FORUMS_TABLE ." f
					WHERE t.topic_id = dl.topic_id
						AND t.forum_id = f.forum_id
						AND dl.user_status = $user_status
			...
			else
			{
				$sql = 'SELECT t.topic_id
					FROM '. TOPICS_TABLE .' t, '. BT_USR_DL_STAT_TABLE ." dl
					WHERE t.topic_id = dl.topic_id
						AND dl.user_status = $user_status
						AND dl.user_id = $dl_uid";
			}
	...
	if ( $search_results != '' )
	{
		...
		if ($dl_search)
		{
			$template->set_filenames(array('body' => 'search_results_downloads.tpl'));
			$template->assign_vars(array('L_SEARCH_DESC' => $l_dl_search_desc));

			$dl_redirect_url = "search_id=dl&amp;dl_status=$user_status";
			$dl_redirect_url .= ($dl_uid != $userdata['user_id']) ? "&amp;dl_uid=$dl_uid" : '';
			$template->assign_vars(array('U_FULL_URL' => $dl_redirect_url));
		...
		for($i = 0; $i < count($searchset); $i++)
		{
			...
			else
			{
				...
				{
					$topic_type = '<span style="color: #DD6900">'. $lang['Topic_Announcement'] .'</span> ';
				}
				...
				//bt
				if ($searchset[$i]['topic_dl_type'] == TOPIC_DL_TYPE_DL)
				{
				...
				$template->assign_block_vars('searchresults', array(
					...
					//bt
					'DL_CHECKBOX' => (isset($dl_checkbox)) ? $dl_checkbox : '',
					//bt end
		...
		if ($dl_search)
		{
			$base_url .= "&amp;dl_search=1&amp;dl_status=$user_status";
			$base_url .= ($dl_uid != $userdata['user_id']) ? "&amp;dl_uid=$dl_uid" : '';
		}
		//bt end
