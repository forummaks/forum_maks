...
if ( $mode == 'topicreview' )
{
	require(FT_ROOT . 'includes/topic_review.php');
...
switch ( $mode )
{
	...
	case 'poll_delete':
		...
		//bt - add [t.topic_dl_type, ]
//	$sql = "SELECT f.*, t.topic_id, t.topic_status, t.topic_type,                  t.topic_first_post_id, t.topic_last_post_id, t.topic_vote, p.post_id, p.poster_id" . $select_sql . "
		$sql = "SELECT f.*, t.topic_id, t.topic_status, t.topic_type, t.topic_dl_type, t.topic_first_post_id, t.topic_last_post_id, t.topic_vote, p.post_id, p.poster_id, p.post_time" . $select_sql . "
...
//snp
// MOD - ¬ топике по€вились новые сообщени€:
// если за врем€ пока вы писали ответ, в топике по€вились новые сообщени€, перед тем как ваше сообщение будет отправлено, выводитс€ предупреждение с обзором этих сообщений
...
//			'NEW_POST_MINI_POST_IMG' => $img_mini_post_new,
...
//upt - add [, $update_post_time]
				submit_post($mode, $post_data, $return_message, $return_meta, $forum_id, $topic_id, $post_id, $poll_id, $topic_type, $bbcode_on, $html_on, $smilies_on, $attach_sig, $bbcode_uid, str_replace("\'", "''", $username), str_replace("\'", "''", $subject), str_replace("\'", "''", $message), str_replace("\'", "''", $poll_title), $poll_options, $poll_length, $update_post_time);
//upt end
			}
			break;
...
if ($topic_dl_type || $post_info['allow_dl_topic'] || $is_auth['auth_mod'])
{
	...
	$dl_ch = (($mode == 'newtopic' && $post_info['dl_type_default']) || ($mode == 'editpost' && $post_data['first_post'] && $topic_dl_type)) ? ' checked="checked" ' : '';
...
$template->assign_vars(array(
	...
	'L_POST_A' => $page_title,
	'L_POST_SUBJECT' => $lang['Post_subject'],
	'U_VIEW_TOPIC' => ($mode != 'newtopic') ? ' ї <a href="' . append_sid("viewtopic.php?" . POST_TOPIC_URL . "=$topic_id") . '" class="nav" style="font-weight:normal">' . $post_info['topic_title'] . '</a>' : '',
