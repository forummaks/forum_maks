...
if ( isset($HTTP_POST_VARS['i_edited']) && $userdata['user_id']==$tor_data['poster_id'] && ($tor_data['topic_check_status']=="3" || $tor_data['topic_check_status']=="4") )
{
	...
	$pm_message='ѕривет, '.$user_to_name .'. \n\n'. 'я исправил недочеты оформлени€ моего релиза: '.'[url='.$phpbb_root_url.'/viewtopic.'.$phpEx.'?t='.$tor_data['topic_id'].']'.$tor_data['topic_title'].'[/url]';
...
if ( isset($HTTP_POST_VARS['topic_check_status']) || isset($HTTP_GET_VARS['topic_check_status']) )
{
	...
	if ($topic_check_status >0 && $tor_data['topic_check_status']!=$topic_check_status)
	{
		...
		switch ($topic_check_status)
		{
			...
			break;
				...
				$pm_message='ѕривет, '.$tor_data['poster_name'] .'. \n\n'. '¬аша тема: '.'[url='.$phpbb_root_url.'/viewtopic.'.$phpEx.'?t='.$tor_data['topic_id'].']'.$tor_data['topic_title'].'[/url]'. ' создана с нарушением правил оформлени€ релизов форума: '.'[url='.$phpbb_root_url.'/viewforum.'.$phpEx.'?f='.$tor_data['forum_id'].']'.$tor_data['forum_name'].'[/url]'. ' в св€зи с этим вам следует исправить недочеты и нажать кнопку "€ исправил"'.$willmove.'. ≈сли у вас возникнут вопросы, вы можете отправить мне личное сообщение.' ;
				send_pm($user_from_id, $user_to_id, $pm_subject, $pm_message);
			break; 
		        case 4:
	...
	//			tracker_unregister($attach_id);
				...
				$pm_message='ѕривет, '.$tor_data['poster_name'] .'. \n\n'. '¬аша тема: '.'[url='.$phpbb_root_url.'/viewtopic.'.$phpEx.'?t='.$tor_data['topic_id'].']'.$tor_data['topic_title'].'[/url]'. ' €вл€етс€ повтором '.$pm_duble_topic_link.'и поэтому будет перенесена в корзину';
				send_pm($user_from_id, $user_to_id, $pm_subject, $pm_message);
